// $Id: rifcalc.js,v 1.4.6.30 2015/04/27 15:14:40 wesleyw Exp $
//  
...
var gRIFFactor  = new Array(gAgeCountRedesign);
...
function initRIFFactor()
{
	gRIFFactor[0] = 0.02857;		//55
	gRIFFactor[1] = 0.02941;		//56
	gRIFFactor[2] = 0.0303;			//57
	gRIFFactor[3] = 0.03125;		//58
	gRIFFactor[4] = 0.03226;		//59
	gRIFFactor[5] = 0.03333;		//60
	gRIFFactor[6] = 0.03448;		//61
	gRIFFactor[7] = 0.03571;		//62
	gRIFFactor[8] = 0.03704;		//63
	gRIFFactor[9] = 0.03846;		//64
	gRIFFactor[10] = 0.04;			//65
	gRIFFactor[11] = 0.04167;		//66
	gRIFFactor[12] = 0.04348;		//67
	gRIFFactor[13] = 0.04545;		//68
	gRIFFactor[14] = 0.04762;		//69
	gRIFFactor[15] = 0.05;			//70
	gRIFFactor[16] = 0.0528;		//71
	gRIFFactor[17] = 0.0540;		//72
	gRIFFactor[18] = 0.0553;		//73
	gRIFFactor[19] = 0.0567;		//74
	gRIFFactor[20] = 0.0582;		//75
	gRIFFactor[21] = 0.0598;		//76
	gRIFFactor[22] = 0.0617;		//77
	gRIFFactor[23] = 0.0636;		//78
	gRIFFactor[24] = 0.0658;		//79
	gRIFFactor[25] = 0.0682;		//80
	gRIFFactor[26] = 0.0708;		//81
	gRIFFactor[27] = 0.0738;		//82
	gRIFFactor[28] = 0.0771;		//83
	gRIFFactor[29] = 0.0808;		//84
	gRIFFactor[30] = 0.0851;		//85
	gRIFFactor[31] = 0.0899;		//86
	gRIFFactor[32] = 0.0955;		//87
	gRIFFactor[33] = 0.1021;		//88
	gRIFFactor[34] = 0.1099;		//89
	gRIFFactor[35] = 0.1192;		//90
	gRIFFactor[36] = 0.1306;		//91
	gRIFFactor[37] = 0.1449;		//92
	gRIFFactor[38] = 0.1634;		//93
	gRIFFactor[39] = 0.1879;		//94
	gRIFFactor[40] = 0.2;			//95
	gRIFFactor[41] = 0.2;			//96
	gRIFFactor[42] = 0.2;			//97
	gRIFFactor[43] = 0.2;			//98
	gRIFFactor[44] = 0.2;			//99
	gRIFFactor[45] = 0.2;			//100
	gRIFFactor[46] = 0.2;			//101
	gRIFFactor[47] = 0.2;			//102
	gRIFFactor[48] = 0.2;			//103
	gRIFFactor[49] = 0.2;			//104
}
...
function findWDSchedule(age, fundsPost1992)
{
   ...
   {   
      var m_age = gAges[i];
      //console.log("Table: " + m_age + " Incoming: " + age);
...
function runRIFGraph(myForm){
	...
	"{\"age\":\"86\",\"value\":\"30000\"},{\"age\":\"88\",\"value\":\"26000\"},{\"age\":\"90\",\"value\":\"21000\"},{\"age\":\"92\",\"value\":\"17000\"},{\"age\":\"94\",\"value\":\"11000\"}," + 
	...
	if(rifArray.length>0){
		...
		$.ajax({ 
		    url: "/t2scr/static/app/tools/rifcalc/genRifCalcImage.action?lang=en&planType=RRIF", 
		    type: "POST",
...
function runLIFGraph(myForm){
	...
	if(lifArray.length>0){
		...
		$.ajax({ 
		    url: "/t2scr/static/app/tools/rifcalc/genRifCalcImage.action?lang=en&planType=LIF", 
		    type: "POST",
		    // The key needs to match your method's input parameter (case-sensitive).
...
function runTFSAGraph(myForm){
	...
	if(tfsaArray.length>0){
		...
		console.log("graph:"+tfsaObjectJsonString);
		$.ajax({ 
		    url: "/t2scr/static/app/tools/rifcalc/genRifCalcImage.action?lang=en&planType=TFSA", 
		    type: "POST",
		    // The key needs to match your method's input parameter (case-sensitive).
...
function generateCalculatorReport(userRole,planType){
	console.log("ok2");
	if("INV"==userRole){
		window.open('/t2scr/static/app/tools/rifcalc/RRIF_CALCULATOR.action?lang=en&cmdString=generatePDF&planType='+planType,'_blank');
	}else{
	...
	//window.open('/t2scr/static/app/tools/rifcalc/RRIF_ILLUSTRATOR.do?lang=en&cmdString=generatePDF','_blank');
}
//
// ---
...
function runRIF(myForm)
{
   ...
   console.log("pre calculation:"+minimumAssume+" annualWithAmount "+annualWithAmount);
...
// Build table sorter pagination sector
   ...
   if(table_size == 0){
	   ...
	   var paginationString = '<table  width="100%" cellpadding="0" cellspacing="0"><tr><td class="table-container-header-sub-title" width="20%"></td><td class="table-container-header-pager pb10" width="80%" align="right" valign="bottom"><table border="0" cellpadding="0" cellspacing="0"><tr><td><div id="rifCalculatorResultPager" class="pagerAGFRedesign pager rifCalculatorPagerForNoneIE"><table cellspacing="0" cellpadding="0" border="0" style="float: right;"><tr><td class="pt8 pr5"><span id="prevButton" class="prev clickable dib vam pager-text" >< PREVIOUS</span></td><td class="pr5" id="rifpagesID"><span>Pages:</span><select class="gotoPage select" id="gotoPageID" style="width: 57px;"></select></td><td class="pt5 pr5"><ul  class="pagecount" > </ul></td><td class="pt8 pr25"><span id="nextButton" class="next clickable dib vam pager-text" >NEXT > </span></td><td class="pt4"><select class="pagesize select" id="perPageID" style="width: 57px;" >'+perPageOptions+'</select><span class="pl5 perPage"> PER PAGE</span></td></td></tr></table></div><div id="rifCalculatorResultPager" class="pagerAGFRedesign pager rifCalculatorPagerForIE"><table cellspacing="0" cellpadding="0" border="0" style="float: right;"><tr><td class="pr5 vam"><span id="prevButton" class="prev clickable pager-text" >< PREVIOUS</span></td><td class="pr5" id="rifpagesID"><select class="gotoPage select" id="gotoPageID" style="width: 57px;"></select></td><td class="pr5"><ul  class="pagecount" style="vertical-align:3px" > </ul></td><td class="pr25 vam"><span id="nextButton" class="next clickable pager-text" >NEXT ></span></td><td class="pr5 pt5"><select class="pagesize select" id="perPageID" style="width: 57px;" >'+perPageOptions+'</select></td><td class="vam"><span class="perPage"> PER PAGE</span></td></tr></table></div></td></tr></table></td></tr></table>';
	   ...
	   .tablesorterPager(pagerOptions)
		...
		$("#rifpagesID").attr("style","display: none");	
   }
...
function writeRRIFHeader2015()
{
       ...
       resultTableStr += "<div class='table-data-head-cell-div fs11n'>Payments</div>"; 
       resultTableStr += "</th>";
       ...
       resultTableStr += "<div class='table-data-head-cell-div fs11n' style='padding-left:10px'>Year</div>";
       resultTableStr += "</th>";
       
       resultTableStr += "<th class='table-data-head-cell table-data-head-cell-colspan-bottom' >";
       resultTableStr += "<div class='table-data-head-cell-div fs11n' style='padding-left:10px'>RRIF Factor Age</div>";
       resultTableStr += "</th>";
       
       resultTableStr += "<th class='table-data-head-cell table-data-head-cell-colspan-bottom'>";
       resultTableStr += "<div class='table-data-head-cell-div fs11n' style='padding-left:10px'>Value($)</div>";
       resultTableStr += "</th>";
       
       resultTableStr += "<th class='table-data-head-cell table-data-head-cell-colspan-bottom' >";
       resultTableStr += "<div class='table-data-head-cell-div fs11n' style='padding-left:10px'>RRIF Minimum Factor(%)</div>";
       resultTableStr += "</th>";
       
       resultTableStr += "<th class='table-data-head-cell table-data-head-cell-colspan-bottom' >";
       resultTableStr += "<div class='table-data-head-cell-div fs11n' style='padding-left:10px'>Annual Amount($)</div>";
       resultTableStr += "</th>";
       
       resultTableStr += "<th class='table-data-head-cell table-data-head-cell-colspan-bottom' >";
       resultTableStr += "<div class='table-data-head-cell-div fs11n' style='padding-left:10px'>Withholding Tax($)</div>";
       resultTableStr += "</th>";
       
       resultTableStr += "<th class='table-data-head-cell table-data-head-cell-colspan-bottom last-table-data-head-cell'>";
       resultTableStr += "<div class='table-data-head-cell-div fs11n' style='padding-left:10px'>Net Per Period($)</div>";
       resultTableStr += "</th>";
...
function writeLIFHeader2015()
{
       ...
       resultTableStr += "<th class='table-data-head-cell table-data-head-cell-colspan-bottom' >";
       resultTableStr += "<div class='table-data-head-cell-div fs11n' style='padding-left:10px'>LIF Factor Age</div>";
       resultTableStr += "</th>";
       ...
       resultTableStr += "<th class='table-data-head-cell table-data-head-cell-colspan-bottom'>";
       resultTableStr += "<div class='table-data-head-cell-div fs11n' style='padding-left:10px'>LIF Minimum Factor(%)</div>";
       resultTableStr += "</th>";
       
       resultTableStr += "<th class='table-data-head-cell table-data-head-cell-colspan-bottom' >";
       resultTableStr += "<div class='table-data-head-cell-div fs11n' style='padding-left:10px'>LIF Maximum Factor(%)</div>";
       resultTableStr += "</th>";
       
       resultTableStr += "<th class='table-data-head-cell table-data-head-cell-colspan-bottom' >";
       resultTableStr += "<div class='table-data-head-cell-div fs11n' style='padding-left:10px'>Annual<br>Amount($)</div>";
       resultTableStr += "</th>";
       
       resultTableStr += "<th class='table-data-head-cell table-data-head-cell-colspan-bottom' >";
       resultTableStr += "<div class='table-data-head-cell-div fs11n' style='padding-left:10px'>Withholding<br>Tax($)</div>";
       resultTableStr += "</th>";
       
       resultTableStr += "<th class='table-data-head-cell table-data-head-cell-colspan-bottom last-table-data-head-cell'>";
       resultTableStr += "<div class='table-data-head-cell-div fs11n' style='padding-left:10px'>Net Per<br>Period($)</div>";
       resultTableStr += "</th>";
...
function writeTFSAHeader2015()
{
       ...
       resultTableStr += "<th class='table-data-head-cell table-data-head-cell-colspan-bottom' >";
       resultTableStr += "<div class='table-data-head-cell-div fs11n' style='padding-left:10px'>Return on Investment($)</div>";
       resultTableStr += "</th>";
...
function validateAnnualAmout(dateOfBirth, expectedROR, toAge, issueDate, firstPayDate, startingBalance, 
        selectionType, deductWHT, annualWithPercent, annualWithAmount, fundsPost1992, province, period,inputAnnualAmount){
	...
	var actualReturn =getActualReturn2015(annualReturn,openYearFraction);
	console.log("Actual Return "+actualReturn);
	console.log("start to draw the table");
	var age=startAge;
	var valueOnJan=parseFloat(startingBalance)+parseFloat(actualReturn);
	var minimum;
	displayYear  = age+birthYear+1;
	minimum=getMinimum(valueOnJan,getRIFFactor2015(age));
	if(parseFloat(inputAnnualAmount)<parseFloat(minimum)){
		alert("Annual amount entered is less than the minimum amount for the year.  Please enter a valid amount");
		return false;
	}
}

//calculate the min first
function preRIFCalculator(dateOfBirth, expectedROR, toAge, issueDate, firstPayDate, startingBalance, 
        selectionType, deductWHT, annualWithPercent, annualWithAmount, fundsPost1992, province, period)
{
	...
	var openYearEnd=getEndDateOfYear(issueDate.getFullYear());	
	var openYearFraction= getYearFraction(issueDate,openYearEnd,0);
	console.log("openYearFraction:"+openYearFraction);
        ...
        selectionType, deductWHT, annualWithPercent, annualWithAmount, fundsPost1992, province, period,isMinimumAssume)
{
	...
	resultTableStr +="<col width=8%><col width=8%><col width=15%><col width=15%><col width=19%><col width=20%><col width=18%>	";
	writeRRIFHeader2015();
	writeTbody();
	...
	{ 
		...
		if(displayFlag==false){
			...
			resultTableStr += convertToMoney(valueOnJan);
			endRowData();
			
			beginRowDataCenter();
			resultTableStr += Math.round(getRIFFactor2015(age)*100*100)/100+"%";
			endRowData();
			
			beginRowData("table-data-right");
			resultTableStr += convertToMoney(minimum);
			endRowData();
			
			beginRowData("table-data-right");
			resultTableStr += convertToMoney(wTax);
			endRowData();
			
			
			beginRowData("table-data-right");
			resultTableStr += convertToMoney(netPerPay);
			endRowData();
		...
		if(isMinimumAssume){
			...
			calcSummaryTableErrorStr +="If the annual payment amount you specify is lower than the required annual minimum payment in any given year, the RRIF Calculator will increase the payment to the required minimum amount.";
		}else{
		...
		calcSummaryTableStr +="<tr><td class='pb10 fs12n summarylabel'>Years Active:</td><td class='fs12n' align='right'>";
		calcSummaryTableStr += yearsActiveCount;
		calcSummaryTableStr +="</td></tr>";
		
		
		calcSummaryTableStr +="<tr><td class='pb10 fs12n summarylabel'>Opening Value:</td><td class='fs12n' align='right'>";
		calcSummaryTableStr +=  "$"+convertToMoney(startingBalance);
		calcSummaryTableStr +="</td></tr>";
		
		calcSummaryTableStr +="<tr><td class='pb10 fs12n summarylabel'>Net Payout:</td><td class='fs12n' align='right'>";
		calcSummaryTableStr += "$"+convertToMoney(totalNetPayouts);
		calcSummaryTableStr +="</td></tr>";
		
		calcSummaryTableStr +="<tr><td class='pb10 fs12n summarylabel'>Taxes Withheld:</td><td class='fs12n' align='right'>";
		calcSummaryTableStr += "$"+convertToMoney(totalWithholdingTaxes);
		calcSummaryTableStr +="</td></tr>";
		
		calcSummaryTableStr +="<tr><td class='pb10 fs12n summarylabel'>Balance Remaining:</td><td class='fs12n' align='right'>";
		calcSummaryTableStr += "$"+convertToMoney(balanceRemaining);
		calcSummaryTableStr +="</td></tr>";
		
		
		calcSummaryTableStr +="<tr><td class='fs12n'>Investment Income:</td><td class='fs12n' align='right'>";
		calcSummaryTableStr += "$"+convertToMoney(balanceRemaining+totalNetPayouts+totalWithholdingTaxes-startingBalance);
		calcSummaryTableStr +="</td></tr>";
	...
	if(rifTableArray.length==0){
		resultTableStr += "<tr class='table-data-row'><td class='table-data-cell table-data-cell-error' colspan='7'>There is no information available for this search.</td></tr>"
	...
	queryParameters+="&totalNetPayouts="+convertToMoney(totalNetPayouts)+"&balanceRemaining="+convertToMoney(balanceRemaining)+"&InvestmentIncome="+convertToMoney(balanceRemaining+totalNetPayouts+totalWithholdingTaxes-startingBalance)+"&totalWithholdingTaxes="+convertToMoney(totalWithholdingTaxes);
	...
	//if(rifTableArray.length>0){
		...
		$.ajax({ 
		    url: "/t2scr/static/app/tools/rifcalc/rrifCalcAction.action?lang=en"+queryParameters, 
		    type: "POST",
		    // The key needs to match your method's input parameter (case-sensitive).
...
function getSmartBeginDate2015(issueDate){
	var firstDayOfYear=stringToDate2015("01/01/"+issueDate.getFullYear());
...
function getDifference2015(annualAnnualPayout,mininum){
	if(annualAnnualPayout>mininum){
		return parseFloat(annualAnnualPayout)-parseFloat(mininum);
	}else{
		return 0;
	}
}
...
function show_eligible_tooltip1(selector){
	var helplink="#"+selector+"_helplink";
	$('#'+selector+'_dialog').dialog(
	     { position: { my: "left top", at: "left bottom", of: helplink, collision: "none" }},
	     { height: 200},
	     { width: 400},
	     { resizable: false},
	     { dialogClass: "tool_tip_custom_dialog"}    
    );
}
...
function preRIFCalculator2017(dateOfBirth, expectedROR, toAge, issueDate, firstPayDate, startingBalance, 
        selectionType, deductWHT, annualWithPercent, annualWithAmount, fundsPost1992, province, period,accountType,firstYearBeginDate,paymentPer)
{
        ...
        selectionType, deductWHT, annualWithPercent, annualWithAmount, fundsPost1992, province, period,isMinimumAssume,isMaximumAssume,accountType,firstYearBeginDate,pjIndex,paymentPer)
{
	...
	resultTableStr +="<col width=6%><col width=8%><col width=15%><col width=13%><col width=13%><col width=15%><col width=15%><col width=15%>	";
	writeLIFHeader2015();
	writeTbody();
	...
	var payTypeReq=0;
	var payTypeCode=0;
	//netPerPay=getNetPerPay(annualWithAmount,period);//request amt
	var requestAmount=0;
	var lifpayment=0;
	...
	{ 
		...
		if(displayFlag==false&& displayFlagForever==false){
			...
			beginRowData("table-data-right");
			resultTableStr += Math.round(minFactor*100*100)/100+"%";
			endRowData();
			
			beginRowData("table-data-right");
			resultTableStr += Math.round(maxFactor*100*100)/100+"%";
			endRowData();
			...
			beginRowData("table-data-right");
			resultTableStr += convertToMoney(netPaymentPerPeriod);
			endRowData();
		...
		if(isMinimumAssume){
			console.log("will show minimum")
			calcSummaryTableErrorStr +="If the annual payment amount you specify is lower than the required annual minimum payment in any given year, the LIF Calculator will increase the payment to the required minimum amount.";
		}else{
			console.log("will not show minimum");
		}
		...
		if(isMaximumAssume){
			...
			calcSummaryTableErrorStr +="If the annual payment amount you specify is higher than the annual maximum payment allowed in any given year, the LIF Calculator will decrease the payment to the maximum amount.";
		}else{
		...
		calcSummaryTableStr +="<tr><td class='fs12n'>Investment Income:</td><td class='fs12n' align='right'>";
		calcSummaryTableStr += "$"+convertToMoney(totalInvestmentIncome);
		calcSummaryTableStr +="</td></tr>";
	...
	queryParameters+="&totalNetPayouts="+convertToMoney(totalNetPayouts)+"&balanceRemaining="+convertToMoney(balanceRemaining)+"&InvestmentIncome="+convertToMoney(totalInvestmentIncome)+"&totalWithholdingTaxes="+convertToMoney(totalWithholdingTaxes)+"&planType=LIF"+"&paymentPer="+paymentPer;
        ...
        selectionType, deductWHT, annualWithPercent, annualWithAmount, fundsPost1992, province, period,isMinimumAssume,accountType,firstYearBeginDate,paymentPer)
{
	...
	resultTableStr = "";
	resultTableStr += "<table id='tablesorter_rifcalculatorbox'  class='tablesorter calculated-data' width='100%' cellspacing='0' cellpadding='0' border='0' style=' table-layout:fixed'>";
	resultTableStr +="<col width=6%><col width=7%><col width=16%><col width=12%><col width=19%><col width=20%><col width=20%>	";
	writeRRIFHeader2015();
	writeTbody();
	...
	console.log("open year fraction 2018"+openYearFraction);
	var annualInterestEarning =getAnnualReturn2015(startingBalance,expectedROR);
	console.log("annual interest earning "+annualInterestEarning);
	var actualInterestEarning =getActualReturn2015(annualInterestEarning,openYearFraction);
	console.log("Actual Interesting Earning "+actualInterestEarning);
	...
	{ 
		...
		if(displayFlag==false&& displayFlagForever==false){
			...
			beginRowDataCenter();
			resultTableStr += Math.round(getRIFFactor2015(factorAge)*100*100)/100+"%";
			endRowData();
...
function runRIF2017(myForm)
{
	...
	queryParameters=null;
	  ...
	  // var firstYearBeginDate=getSmartBeginDate2015(issueDate);
	   ...
	   var maxpjvalue=  gPensionJurisdictionMaxValue[pjIndex];
	   console.log("pjvalue min: "+minpjvalue+" max "+maxpjvalue);
	   paymentPer=prepareNumber(myForm.txtAmount.value);
	   annualWithAmount=getAnnualPayment2017(paymentPer,period)
	   var minimumAssume=preRIFCalculator2017(dateOfBirth, adjustedROR, lastAge, issueDate, firstYearEndDate,startingBalance, selectionType, deductWHT,annualWithPercent, annualWithAmount, post1992, province, period,accountType,firstYearBeginDate,paymentPer);
	   ...
	   queryParameters="&issueDate="+issdate+"&startBalance="+startingBalance+"&DOB="+dob+"&quebecResident="+myForm.quebecResident.value+"&paymentType="+myForm.paymentType.value+"&paymentFrequency="+paymentFreqName+"&deductWHT="+deductWHT+"&ror="+myForm.txtROR.value+"&pjName="+pjName+"&atName="+atName+"&planType=RRIF"+"&paymentPer="+paymentPer;
	...
	// Build table sorter pagination sector
	   ...
	   if(table_size == 0){
		   ...
		   $("#result_summary_container").addClass("noValue");
		   $('#rif-calculator-result-table').html(resultTableStr);
		   console.log("table size is not 0");
	   }else{
		   if(table_size<=10 ){
		 	 perPageOptions =  perPageOptions +  "<option selected='selected' value='10'>10</option>";
		   ...
		   console.log("table size is not 0");
		   $("#result_summary_container").removeClass("noValue");
		   var paginationString = '<table  width="100%" cellpadding="0" cellspacing="0"><tr><td class="table-container-header-sub-title" width="20%"></td><td class="table-container-header-pager pb10" width="80%" align="right" valign="bottom"><table border="0" cellpadding="0" cellspacing="0"><tr><td><div id="rifCalculatorResultPager" class="pagerAGFRedesign pager rifCalculatorPagerForNoneIE"><table cellspacing="0" cellpadding="0" border="0" style="float: right;"><tr><td class="pt8 pr5"><span id="rrifprevButton" class="prev clickable dib vam pager-text" >< PREVIOUS</span></td><td class="pr5" id="rrifpagesID"><span>Pages:</span><select class="gotoPage select" id="rrifgotoPageID" style="width: 57px;"></select></td><td class="pt5 pr5"><ul  class="pagecount" > </ul></td><td class="pt8 pr25"><span id="rrifnextButton" class="next clickable dib vam pager-text" >NEXT > </span></td><td class="pt4"><select class="pagesize select" id="rrifperPageID" style="width: 57px;" >'+perPageOptions+'</select><span class="pl5 perPage"> PER PAGE</span></td></td></tr></table></div><div id="rifCalculatorResultPager" class="pagerAGFRedesign pager rifCalculatorPagerForIE"><table cellspacing="0" cellpadding="0" border="0" style="float: right;"><tr><td class="pr5 vam"><span id="rrifprevButton" class="prev clickable pager-text" >< PREVIOUS</span></td><td class="pr5" id="rrifpagesID"><select class="gotoPage select" id="rrifgotoPageID" style="width: 57px;"></select></td><td class="pr5"><ul  class="pagecount" style="vertical-align:3px" > </ul></td><td class="pr25 vam"><span id="rrifnextButton" class="next clickable pager-text" >NEXT ></span></td><td class="pr5 pt5"><select class="pagesize select" id="rrifperPageID" style="width: 57px;" >'+perPageOptions+'</select></td><td class="vam"><span class="perPage"> PER PAGE</span></td></tr></table></div></td></tr></table></td></tr></table>';
		   //var topBarString = '<div class="table-top-bar"><img border="0" src="/t2scr/sharedDeployedT2scrWeb/images/dot.gif" width="1" height="5" /></div>';
		   // Set result to result DIV
		   $("#rif-calculator-pager").attr("style","display: ");
		   $('#rif-calculator-pager').html(paginationString);
...
function runLIF2017(myForm)
{
	...
	queryParameters=null;	   
	 ...
	 //  var firstYearEndDate=getSmartFirstPaymentDate2015(issueDate,period);
	  ...
	  // var firstYearBeginDate=getSmartBeginDate2015(issueDate);
	   ...
	   console.log("pre min calculation:"+minimumAssume+"pre max calculation:"+maximumAssume+" payment per period "+paymentPer);
	   console.log("dateOfBirth"+ dateOfBirth+"adjustedROR"+adjustedROR+"lastAge"+lastAge+"issueDate"+issueDate+"firstYearEndDate"+firstYearEndDate+"startingBalance"+startingBalance+"selectionType"+selectionType+"deductWHT"+deductWHT+"annualWithPercent"+annualWithPercent+"annualWithAmount"+annualWithAmount+ "post1992"+post1992+"province"+province+"period"+period);
	   queryParameters="&issueDate="+issdate+"&startBalance="+startingBalance+"&DOB="+dob+"&quebecResident="+myForm.quebecResident.value+"&paymentType="+myForm.paymentType.value+"&paymentFrequency="+paymentFreqName+"&deductWHT="+deductWHT+"&ror="+myForm.txtROR.value+"&pjName="+pjName+"&atName="+atName+"&planType=LIF";
	...
	// Build table sorter pagination sector
	   ...
	   if(table_size == 0){
		   ...
		   $("#result_summary_container").removeClass("noValue");
		   var paginationString = '<table  width="100%" cellpadding="0" cellspacing="0"><tr><td class="table-container-header-sub-title" width="20%"></td><td class="table-container-header-pager pb10" width="80%" align="right" valign="bottom"><table border="0" cellpadding="0" cellspacing="0"><tr><td><div id="lifCalculatorResultPager" class="pagerAGFRedesign pager lifCalculatorPagerForNoneIE"><table cellspacing="0" cellpadding="0" border="0" style="float: right;"><tr><td class="pt8 pr5"><span id="lifprevButton" class="prev clickable dib vam pager-text" >< PREVIOUS</span></td><td class="pr5" id="lifpagesID"><span>Pages:</span><select class="gotoPage select" id="lifgotoPageID" style="width: 57px;"></select></td><td class="pt5 pr5"><ul  class="pagecount" > </ul></td><td class="pt8 pr25"><span id="lifnextButton" class="next clickable dib vam pager-text" >NEXT > </span></td><td class="pt4"><select class="pagesize select" id="lifperPageID" style="width: 57px;" >'+perPageOptions+'</select><span class="pl5 perPage"> PER PAGE</span></td></td></tr></table></div><div id="lifCalculatorResultPager" class="pagerAGFRedesign pager lifCalculatorPagerForIE"><table cellspacing="0" cellpadding="0" border="0" style="float: right;"><tr><td class="pr5 vam"><span id="lifprevButton" class="prev clickable pager-text" >< PREVIOUS</span></td><td class="pr5" id="lifpagesID"><select class="gotoPage select" id="lifgotoPageID" style="width: 57px;"></select></td><td class="pr5"><ul  class="pagecount" style="vertical-align:3px" > </ul></td><td class="pr25 vam"><span id="lifnextButton" class="next clickable pager-text" >NEXT ></span></td><td class="pr5 pt5"><select class="pagesize select" id="lifperPageID" style="width: 57px;" >'+perPageOptions+'</select></td><td class="vam"><span class="perPage"> PER PAGE</span></td></tr></table></div></td></tr></table></td></tr></table>';
		   //var topBarString = '<div class="table-top-bar"><img border="0" src="/t2scr/sharedDeployedT2scrWeb/images/dot.gif" width="1" height="5" /></div>';
		   // Set result to result DIV
        ...
        selectionType, deductWHT, annualWithPercent, annualWithAmount, fundsPost1992, province, period,isMinimumAssume,accountType,firstYearBeginDate,pjIndex,paymentPer,currentAge,retireAge)
{
	...
	{ 
		...
		if(displayFlag==false){
			...
			beginRowData("table-data-right");
			resultTableStr += convertToMoney(tfsainterest);
			endRowData();
						
			beginRowData("table-data-right");
			resultTableStr += convertToMoney(netPaymentPerPeriod*period);
			endRowData();
		...
		calcSummaryTableStr +="<tr><td class='pb10 fs12n summarylabel'>Net Withdrawal:</td><td class='fs12n' align='right'>";
		calcSummaryTableStr += "$"+convertToMoney(totalNetPayouts);
		calcSummaryTableStr +="</td></tr>";
...
function runTFSA2017(myForm)
{
	console.log("run TFSA 2019");   
	 ...
	 var yearsToRetirement=retireAge-currentAge;
	  ...
	  /* if (issueDate == null || issueDate == false)
	   ...
	   var lastAge = prepareNumber("100");
	   var startingBalance = prepareNumber(myForm.txtPlanValue.value);
	   var selectionType = 1;//identifySelectionType(myForm.paymentType.value);
	   console.log("startingBalance"+startingBalance+" selectionType "+selectionType);
	   var period = prepareNumber(myForm.paymentFreq.value);
	   console.log("period is "+period+" mapsize"+MAP_SIZE);
	   paymentFreqName="";
	   ...
	   console.log("tfsa account type 2018:"+accountType);
	   if(accountType=="1"){
	...
	// Build table sorter pagination sector
	   ...
	   if(table_size == 0){
		   ...
		   $("#result_summary_container").removeClass("noValue");
		   var paginationString = '<table  width="100%" cellpadding="0" cellspacing="0"><tr><td class="table-container-header-sub-title" width="20%"></td><td class="table-container-header-pager pb10" width="80%" align="right" valign="bottom"><table border="0" cellpadding="0" cellspacing="0"><tr><td><div id="tfsaCalculatorResultPager" class="pagerAGFRedesign pager tfsaCalculatorPagerForNoneIE"><table cellspacing="0" cellpadding="0" border="0" style="float: right;"><tr><td class="pt8 pr5"><span id="tfsaprevButton" class="prev clickable dib vam pager-text" >< PREVIOUS</span></td><td class="pr5" id="tfsapagesID"><span>Pages:</span><select class="gotoPage select" id="tfsagotoPageID" style="width: 57px;"></select></td><td class="pt5 pr5"><ul  class="pagecount" > </ul></td><td class="pt8 pr25"><span id="tfsanextButton" class="next clickable dib vam pager-text" >NEXT > </span></td><td class="pt4"><select class="pagesize select" id="tfsaperPageID" style="width: 57px;" >'+perPageOptions+'</select><span class="pl5 perPage"> PER PAGE</span></td></td></tr></table></div><div id="tfsaCalculatorResultPager" class="pagerAGFRedesign pager tfsaCalculatorPagerForIE"><table cellspacing="0" cellpadding="0" border="0" style="float: right;"><tr><td class="pr5 vam"><span id="tfsaprevButton" class="prev clickable pager-text" >< PREVIOUS</span></td><td class="pr5" id="tfsapagesID"><select class="gotoPage select" id="tfsagotoPageID" style="width: 57px;"></select></td><td class="pr5"><ul  class="pagecount" style="vertical-align:3px" > </ul></td><td class="pr25 vam"><span id="tfsanextButton" class="next clickable pager-text" >NEXT ></span></td><td class="pr5 pt5"><select class="pagesize select" id="tfsaperPageID" style="width: 57px;" >'+perPageOptions+'</select></td><td class="vam"><span class="perPage"> PER PAGE</span></td></tr></table></div></td></tr></table></td></tr></table>';
		   //var topBarString = '<div class="table-top-bar"><img border="0" src="/t2scr/sharedDeployedT2scrWeb/images/dot.gif" width="1" height="5" /></div>';
		   // Set result to result DIV
